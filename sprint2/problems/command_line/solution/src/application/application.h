...
namespace app {
    ...
    public:
        ...
        std::tuple<auth::Token, Player::Id> JoinGame(const std::string& player_name, const model::Map::Id& id);    //Залогинить игорька
        ...
        std::shared_ptr<GameSession> GameSessionById(const model::Map::Id& id) const noexcept;           //Геттер сессии по айди
    ...
    private:
        ...
        MapIdToSessionIndex map_id_to_session_index_;                                                   //из гейм сессии перенос, после рефактора
