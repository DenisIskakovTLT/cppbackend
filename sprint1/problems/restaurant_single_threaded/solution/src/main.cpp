...
public:
    ...
    void Execute() {
        logger_.LogMessage("Start to prepare..."sv);
        RoastCutlet();
        if (with_onion_) {
            MarinadeOnion();
        }
...
private:
    ...
    void CheckReadiness(sys::error_code ec) {
        if (delivered_) {       //доставлено уже, вываливаемся
            
            return;
        }
        if (ec) {
            return Deliver(ec);     //Ошибка
        }
    ...
    void Deliver(sys::error_code ec) {
        delivered_ = true;                  //Чтоб 2 раза не доставили один и тот же заказ
        handler_(ec, id_, ec ? nullptr : &hamburger_);
    }

    /*Тут нодискард, т.к. результат 100% нужен*/
    [[nodiscard]] bool CanAddOnion() const {
        return hamburger_.IsCutletRoasted() && onion_marinaded_ && !hamburger_.HasOnion(); //Тут проверка, что котлета обжарена и лук замаринован
    }

    /*Тут нодискард, т.к. результат 100% нужен*/
    [[nodiscard]] bool IsReadyToPack() const {
        return hamburger_.IsCutletRoasted() && (!with_onion_ || hamburger_.HasOnion());     //упаковывать можно когда всё готово, обжарено идобавлено
    }
