...
public:
    ...
    void StartBake(GasCooker& cooker, Handler handler) {
        ...
        if (baking_start_time_) {       //Тут проверяем на повторный вызов
            throw std::logic_error("Baking already started");
        }
 
        baking_start_time_ = Clock::now(); // Лок на повторный вызов

        gas_cooker_mtx_ = GasCookerLock{ cooker.shared_from_this() };       //что-то типа забрали мютекс

        cooker.UseBurner([self = shared_from_this(), handler = std::move(handler)]{             //используем шаред, чтоб объект не схлопнулся раньше времени
            
        self->baking_start_time_ = Clock::now();        //Время начала выпекания

        self->logger_.LogMessage("UseBurner on thread #"s + getThreadIdStr());//жДЁМ ПОКА НЕ ЗАКОНЧИТСЯ ВЫПЕКАНИЕ
        handler();
            });
    }
    ...
    void StopBaking() {
        ...
        if (!baking_start_time_) {                                  //Проверяем а начали ли вообще готовить
            throw std::logic_error("Frying has not started");
        }
        
        if (baking_end_time_) {                                     //Проверка на повторный вызов
            throw std::logic_error("Frying has stopped");
        }
        
        baking_end_time_ = Clock::now();                            //Что-то типа индикатора повторого выщова
        
        gas_cooker_mtx_.Unlock();                                   //Отдаём обратно
        logger_.LogMessage("Gas cooker free on thread #"s + getThreadIdStr());
    }

    // Информирует, испечён ли хлеб
    bool IsCooked() const noexcept {
        return baking_start_time_.has_value() && baking_end_time_.has_value();              //Если есть время старта для начала и конца выпекания, значит выпекли
    }
...
private:
    ...
    GasCookerLock gas_cooker_mtx_;                 //Что-то типа мютекса для плиты
