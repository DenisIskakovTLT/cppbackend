    ...
    private:
        ...
        void CheckReadiness(sys::error_code ec) {
            ...
            if (ec) {
                ...
                return Deliver(ec);         //Нельзя выполнить заказ
            }

            
            if (CanAddOnion()) {                                //добавляем лук
                logger_.LogMessage("Add onion"sv);
                hamburger_.AddOnion();
            }

            
            if (IsReadyToPack()) {          //Всё ок пакуем
                Pack();
            }
        }
        ...
        void Deliver(sys::error_code ec) {
            ...
            delivered_ = true;              //Чтобы игнорировать потом повторную доставку
...
int main() {
    ...
    for (const auto& [id, o] : orders) {
        ...
        assert(o.hamburger.HasOnion() == (id % 2 != 0));
    }
}